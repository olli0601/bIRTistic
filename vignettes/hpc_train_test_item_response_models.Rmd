---
title: "Train and test partial credit model on HPC"
date: "`r Sys.Date()`"
output: 
...
if (grepl("^/rds/general/user/or105", here::here())) {
    ...
    dir_out <- "/rds/general/user/or105/home/WORK/2025_project_Hope_Groups/colombia-validate-pcm-251224"
} else {
    # Working locally
    on_hpc <- FALSE
    dir_data <- "/Users/or105/Library/CloudStorage/OneDrive-ImperialCollegeLondon/OR_Work/2025/2025_project_Hope_Groups/data"
    dir_out <- "/Users/or105/sandbox/colombia-validate-pcm-251224"
}
...
# Define job name
job_name <- paste0("validate_pcm_", format(Sys.time(), "%Y-%m-%d-%H-%M-%S"))
```
...
```{r evaluate-results, include=TRUE, eval=FALSE, message=FALSE, echo=TRUE, warning=FALSE}
tmp <- list.files(dir_out, pattern = "validation_summary", full.names = TRUE)
results <- vector("list", length(tmp))
for(i in seq_along(tmp)) {
    results[[i]] <- read.csv(tmp[i])
}
results <- as.data.table(do.call(rbind, results))
setkey(results, split_idx)
```
