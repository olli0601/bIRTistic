...
dir.out <- file.path(dir.home, 'analysis-250612')
file.data <- file.path(dir.data,'JORDAN_DATASET_4_Olli.csv')
set.seed(42L)
```
...
# Binomial model - intervention versus control

```{r Subset data for binomial model, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
# select 0-7 days outcome
ddow <- subset(dp2, variable == 'y_violence_against_children')

# select data to compare intervention/endline to control/baseline
ddow <- subset(ddow, (arm == 1 & time == 1) | (arm == 0 & time == 0))

ddow[, oid := seq_len(nrow(ddow))]

set(ddow, NULL, 'arm', ddow[, arm + 1L])

ddow <- ddow[order(pid, arm, time, item)]
```

```{r stan_code_binomial_logistic, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
binomial_logit_txt <- "
functions{
...
data{
  ...
  int<lower=3> K; // max number of occurrences, ie K=7 for days in week 
...
transformed parameters{
  ...
  logit_p = beta_0 + beta_arm[arm_of_obs] + beta_fac_sd * beta_fac[facilitator_of_obs];
}
model{
  // likelihood
  ...
  // priors
  beta_0 ~ normal(0, 3.5); // gtools::inv.logit(2*3.5) = 0.9991
...
# compile the model
binomial_logit_filename <- cmdstanr::write_stan_file(
  gsub('\t',' ', binomial_logit_txt),
  dir = dir.out,
  basename = NULL,
...
# compile Stan model
binomial_logit_compiled <- cmdstanr::cmdstan_model(binomial_logit_filename)
```

```{r Run binomial model, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
# define data in format needed for model specification
stan_data <- list()
stan_data$N <- nrow(ddow)
stan_data$P <- 2L
stan_data$F <- max(ddow$fid)
stan_data$K <- max(ddow$value)
stan_data$y <- ddow$value
stan_data$arm_of_obs <- ddow$arm
stan_data$facilitator_of_obs <- ddow$fid

# sample
binomial_logit_fit <- binomial_logit_compiled$sample(
  data = stan_data,
  seed = 123,
  chains = 2,
  parallel_chains = 2,
...
# save output to RDS
binomial_logit_fit$save_object(file = file.path(dir.out, "violence_binomial_logit_m1_stan.rds"))
```


```{r check mixing convergence for Binomial model, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
#	check convergence and check mixing
#	get 95% credible intervals
tmp <- binomial_logit_fit$summary(
  variables = c('beta_0','beta_arm_m1','beta_fac_m1','beta_fac_sd'),
...
p <- p + theme_bw()
ggsave(file = file.path(dir.out,'violence_binomial_logit_m1_worsttrace.png'), 
       plot = p, 
       h = 10, 
       w = 8
       )
...
ggsave(file = file.path(dir.out,'violence_binomial_logit_m1_pairsplot.png'), 
       plot = p, 
       h = 30, 
       w = 30, 
       limitsize = FALSE
...
```{r Extract event probs for each arm, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
# extract model parameters: arm effect sizes and cut points
po <- binomial_logit_fit$draws(
  variables = c('beta_0','beta_arm'),
  inc_warmup = FALSE,
  format = "draws_df"
  )
po <- as.data.table(po)
...
po[, p_arm1 := gtools::inv.logit(beta_0 + beta_arm_1)]
po[, p_arm2 := gtools::inv.logit(beta_0 + beta_arm_2)]
po <- subset(po, select = c(.draw, p_arm1,  p_arm2))
po <- data.table::melt(po, id.vars = '.draw', value.name = 'prob')
po[, arm := as.integer(gsub('p_arm([0-9])','\\1',variable))]
tmp <- unique( subset(ddow, select = c(arm, arm_label)) )
po <- merge(po, tmp, by = c('arm'))
```

```{r Make histogram of event probs across arms, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
pos <- po[, 
...
pos <- data.table::dcast(pos, arm + arm_label ~ summary_name, value.var = 'summary_value')

p <- ggplot(data = pos, aes( x = arm_label, fill = arm_label)) +
  ...
  labs(x = '\nViolence against Children\nDays per week', y = 'group-level posterior probability', fill = '') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45,vjust = 1,hjust = 1),
        legend.position = 'bottom'
        )
ggsave(file = file.path(dir.out,'violence_binomial_logit_m1_event_probs_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 5
       )
...
```{r Make risk difference in outcome, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
po2 <- data.table::dcast(po, .draw ~ arm_label, value.var = 'prob')
po2[, diff := Intervention - Control]

po2s <- po2[, 
            list( summary_value = quantile(diff, prob = c(0.025, 0.25, 0.5, 0.75, 0.975)),
                  summary_name = c('q_lower','iqr_lower','median','iqr_upper','q_upper') 
                  ), 
            ]
po2s[, value_label := 'Intervention-Control']
po2s <- data.table::dcast(po2s, value_label ~ summary_name, value.var = 'summary_value')
...
ggsave(file = file.path(dir.out,'violence_binomial_logit_m1_event_prob_differences_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 5
       )
...
```{r Make percent change in outcome, include=TRUE, eval=TRUE, echo=TRUE, tidy=FALSE, cache=TRUE}
po2 <- data.table::dcast(po, .draw ~ arm_label, value.var = 'prob')
po2[, perc.ch := 1 - Intervention / Control]

po2s <- po2[, 
            list( summary_value = quantile(perc.ch, prob = c(0.025, 0.25, 0.5, 0.75, 0.975)),
                  summary_name = c('q_lower','iqr_lower','median','iqr_upper','q_upper') 
                  ), 
            ]
po2s[, value_label := 'Intervention vs Control']
po2s <- data.table::dcast(po2s, value_label ~ summary_name, value.var = 'summary_value')
...
ggsave(file = file.path(dir.out,'violence_binomial_logit_m1_percent_reduction_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 5
       )
...
# Ordered Cat model - intervention versus control

```{r Subset data for ordered cat model, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
...
#
ordered_categorical_logit_m1_txt <- "
functions{
  matrix sum2zero_generating_matrix(int K) {
    matrix[K, K] A = diag_matrix(rep_vector(1, K));
    for (i in 1:K - 1) A[K, i] = -1;
...
ordered_categorical_logit_m1_filename <- cmdstanr::write_stan_file(
  gsub('\t',' ', ordered_categorical_logit_m1_txt),
  dir = dir.out,
  basename = NULL,
  force_overwrite = FALSE,
  hash_salt = ""
...
# compile Stan model
ordered_categorical_logit_m1_compiled <- cmdstanr::cmdstan_model(ordered_categorical_logit_m1_filename)
```
...
# sample
ordered_categorical_logit_m1_fit <- ordered_categorical_logit_m1_compiled$sample(
  data = stan_data,
  seed = 123,
  chains = 2,
  parallel_chains = 2,
...
# save output to RDS
ordered_categorical_logit_m1_fit$save_object(file = file.path(dir.out, "parentalmh_ordered_categorical_logit_m1_stan.rds"))
```
...
#	check convergence and check mixing
#	get 95% credible intervals
tmp <- ordered_categorical_logit_m1_fit$summary(
...
# extract samples
po <- ordered_categorical_logit_m1_fit$draws(
  variables = c("lp__", worst_var),
  inc_warmup = TRUE,
  format = "draws_array"
  )
...
p <- p + theme_bw()
ggsave(file = file.path(dir.out,'parentalmh_ordered_categorical_logit_m1_worsttrace.png'), 
       plot = p, 
       h = 10, 
       w = 8
       )
...
ggsave(file = file.path(dir.out,'parentalmh_ordered_categorical_logit_m1_pairsplot.png'), 
       plot = p, 
       h = 30, 
       w = 30, 
       limitsize = FALSE
...
ggsave(file = file.path(dir.out,'parentalmh_ordered_categorical_logit_m1_categorial_probs_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 8
       )
...
ggsave(file = file.path(dir.out,'parentalmh_ordered_categorical_logit_m1_categorial_differences_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 8
       )
...
ggsave(file = file.path(dir.out,'parentalmh_ordered_categorical_logit_m1_categorial_percent_reduction_barplot.png'), 
       plot = p, 
       h = 8, 
       w = 8
       )
...
stan <- stan_lmer(as.formula(formula), 
                  iter = 100, 
                  chains = 4, 
                  cores = 1, 
                  data = final_dataset,  
