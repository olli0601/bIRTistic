...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "response_frequencies.png")),
  plot = p,
  h = 40,
  w = 12
)
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "response_frequencies.pdf")),
  plot = p,
  h = 40,
  w = 12
)
...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "accrueing_number_participants.png")),
  plot = p,
  h = 7,
  w = 7
)
...
# Compile Stan model
ugpcm_m1_compiled <- cmdstanr::cmdstan_model(stan_file)
```
...
for (i in seq_len(nrow(di)))
{
  ...
  if (0) {
    ...
    upgcm_m1_all2_fit$save_object(
      file = file.path(dir.out, paste0(file.prefix, "_", i, "_stan.rds"))
    )
  }
  if (1) {
    upgcm_m1_all2_fit <- NULL
  ...
  p <- p + theme_bw()
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_worsttrace.png")),
    plot = p,
    h = 10,
    w = 20
  )
  ...
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_intervals.png")),
    plot = p,
    h = 50,
    w = 8,
    limitsize = FALSE
  ...
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_areas.png")),
    plot = p,
    h = 150,
    w = 8,
    limitsize = FALSE
  ...
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_ppcheck.png")),
    p,
    w = 20,
    h = 20
  )
  ...
  p <- ggplot(
    ...
    guides(fill = guide_legend(ncol = 3))
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_probs_barplot_v2.png")),
    plot = p,
    h = 40,
    w = 12
  )
  ggsave(
    file = file.path(dir.out, paste0(file.prefix, "_", i, "_probs_barplot_v2.pdf")),
    plot = p,
    h = 40,
    w = 12
  )
...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_11-11_compare_intervals.pdf")),
  plot = p,
  h = 8,
  w = 100,
  limitsize = FALSE
...
saveRDS(ipos,
  file = file.path(dir.out, paste0(file.prefix, "_primary_endpoints_b.rds"))
)
```
...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_percentchanges_over_time.png")),
  plot = p,
  h = 16,
  w = 14
)
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_percentchanges_over_time.pdf")),
  plot = p,
  h = 16,
  w = 14
)
...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_differences_over_time.png")),
  plot = p,
  h = 16,
  w = 14
)
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_differences_over_time.pdf")),
  plot = p,
  h = 16,
  w = 14
)
...
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_relativeimprovement_over_time.png")),
  plot = p,
  h = 12,
  w = 14
)
ggsave(
  file = file.path(dir.out, paste0(file.prefix, "_relativeimprovement_over_time.pdf")),
  plot = p,
  h = 12,
  w = 14
)
