---
title: "Colombia use ordered logistic 2 category model"
date: "2025-12-26"
output: 
...
if (grepl("^/rds/general/user/or105", here::here())) {
    ...
    dir_out <- "/rds/general/user/or105/home/WORK/2025_project_Hope_Groups/colombia-run-orderedlogistic-251222"
    dir_out_cm <- "/rds/general/user/or105/home/WORK/2025_project_Hope_Groups/colombia-run-creditmodel-251120"
} else {
    # Working locally
    on_hpc <- FALSE
    dir_data <- "/Users/or105/Library/CloudStorage/OneDrive-ImperialCollegeLondon/OR_Work/2025/2025_project_Hope_Groups/data"
    dir_out <- "/Users/or105/sandbox/colombia-run-orderedlogistic-251222"
    dir_out_cm <- "/Users/or105/sandbox/colombia-run-creditmodel-251120"
}
...
# Define output relateed filenames and prefixes
output_file_prefix <- file.path(dir_out, "ordlog_1")
output_file_prefix_cm <- file.path(dir_out_cm, "creditmodel_1")
job_name <- paste0("run-orderedlogistic_", format(Sys.time(), "%Y-%m-%d-%H-%M-%S"))
...
```{r compile-stan-model, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
stan_file <- here::here("src", "stan", "ordered_logit_2cats_v251222_threading.stan")
...
```{r compile-stan-model, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
stan_file <- here::here("src", "stan", "credit_model_2cats_v251120_threading.stan")
...
```{r cm-fit-on-training-data, include=TRUE, eval=TRUE, message=FALSE, echo=TRUE, warning=FALSE, tidy=TRUE}
