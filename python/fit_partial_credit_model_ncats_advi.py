...
def fit_partial_credit_model_ncats_advi(
    dit: pd.DataFrame,
    dcati: pd.DataFrame,
    ...
    if can_resume:
        ...
        timing_data = pd.DataFrame({'total_minutes': [elapsed_time / 60]})
    ...
    if with_additional_analyses and not os.path.exists(f"{output_file_prefix}_intervals.pdf"):
        ...
        # Partial credit ncats parameters block has: latent_factor_unit, latent_factor_beta,
