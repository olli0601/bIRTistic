...
class OrderedLogit(IRTModel):
    ...
    def eval_log_prior(params):
        """Pointwise log p(theta) for the ordered logit ncats priors."""
        return _model_namespace()['get_prior_of_ordered_logit_ncats'](params)

    @staticmethod
    def eval_outcome_for_endpoint(data, params):
        """The ordered-logit .pyro module does not yet expose a standalone
        ``get_ordered_prob_of_ordered_logit_ncats`` callable; raising loudly
    ...
    def make_stan_data(self, dcati: pd.DataFrame, x_formula: str,
        ...
        """Build stan_data and design-matrix metadata for ordered logit
