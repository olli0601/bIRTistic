"""
Model fitting functions for the ordered logit (ncats) Bayesian IRT model.

Three inference back-ends are provided as separate top-level functions:

- ``fit_ordered_logit_model_ncats_pyrosvi``: SVI with NumPyro.
- ``fit_ordered_logit_model_ncats_stanadvi``: ADVI with Stan (cmdstanpy).
- ``fit_ordered_logit_model_ncats_stanhmc``: HMC with Stan (cmdstanpy).

The private helper ``_fit_ordered_logit_make_stan_data`` (formerly in
...
def fit_ordered_logit_model_ncats_stanhmc(
    dit: pd.DataFrame,
    dcati: pd.DataFrame,
    output_file_prefix: str,
    stan_file: Optional[str] = None,
