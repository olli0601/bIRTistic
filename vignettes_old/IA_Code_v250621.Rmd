...
# save output to RDS
binomial_logit_fit$save_object(file = file.path(dir_out, "violence_binomial_logit_m1_stan.rds"))
```
...
p <- p + theme_bw()
ggsave(
  file = file.path(dir_out, "violence_binomial_logit_m1_worsttrace.png"),
  plot = p,
  h = 10,
  w = 8
)
...
ggsave(
  file = file.path(dir_out, "violence_binomial_logit_m1_pairsplot.png"),
  plot = p,
  h = 30,
  w = 30,
  limitsize = FALSE
...
ggsave(
  file = file.path(dir_out, "violence_binomial_logit_m1_event_probs_barplot.png"),
  plot = p,
  h = 8,
  w = 5
)
...
ggsave(
  file = file.path(dir_out, "violence_binomial_logit_m1_event_prob_differences_barplot.png"),
  plot = p,
  h = 8,
  w = 5
)
...
po2s <- po2[
  ,
  list(
    summary_value = quantile(perc.ch, prob = c(0.025, 0.25, 0.5, 0.75, 0.975)),
    summary_name = c("q_lower", "iqr_lower", "median", "iqr_upper", "q_upper")
  ),
]
...
ggsave(
  file = file.path(dir_out, "violence_binomial_logit_m1_percent_reduction_barplot.png"),
  plot = p,
  h = 8,
  w = 5
)
...
ggsave(
  file = file.path(dir_out, "parentalmh_ordcat_logit_m1_categorial_percent_reduction_barplot.png"),
  plot = p,
  h = 8,
  w = 8
)
