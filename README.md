...
Rscript -e "packages <- c('argparse', 'bayesplot', 'bookdown', 'GGally', 'ggplot2', 'ggsci', 'here', 'hexbin', 'jsonlite', 'kableExtra', 'knitr', 'loo', 'polycor', 'posterior', 'purrr','rmarkdown', 'scales'); install.packages(packages[!packages %in% installed.packages()[,'Package']], repos = 'https://cloud.r-project.org', ask = false)"
```
