...
# rho declared upfront (short + long label): MSPSS perceived-support relative gain, baseline->endline
RHO_SPECS = [{'rho_id': 1, 'rho_label': 'support_gain', 'reduction': 'mean', 'compare': 'ratio',
...
# construct / construct_long come from read_data_refuge_ed (per-subscale: Fam/Fri/SO)
dit.to_csv(f"{dir_out}/{file_prefix}_1_data_dit.csv", index=False)

# linked participants only (present at both times)
both = dp.groupby('pid').group.nunique(); linked = set(both[both >= 2].index)
