...
# Output directories
dir_out_pcm = "/Users/or105/sandbox/bIRTistic/py-colombia-partial_credit-260430-vanilla-advi-hmc-pyro-OOchecker"
...
item_time_df = (
    dp1_col[['item_type', 'item_label', 'group']]
    .drop_duplicates()
...
# Merge item_group_id
dp1_col = dp1_col.merge(item_time_df, on=['item_label', 'group', 'item_type'], how='left')
...
# Sort and create oid
dp1_col = dp1_col.sort_values(['item_type_id', 'pid', 'group', 'item_label']).reset_index(drop=True)
...
_pcm = PartialCreditModel(
    dit=dit_col, dcati=dp1_col, x_formula="~ group - 1", seed=seed,
)
...
_emp_n = dp1_col.groupby(['group_label', 'item_label', 'y']).size().reset_index(name='_emp_n')
_emp_tot = dp1_col.groupby(['group_label', 'item_label']).size().reset_index(name='_emp_total')
...
tmp_emp = tmp_emp.merge(
    dit_col[['item_type_id', 'item_label', 'construct_long', 'item_label_short']],
