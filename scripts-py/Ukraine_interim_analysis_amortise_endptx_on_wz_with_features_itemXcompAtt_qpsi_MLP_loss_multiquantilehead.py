...
# Per-item target standardisation (§14.1 CG-MH resolution fix). Divide
# pps_ratio_x by per-item std computed on the training pool so all items
# contribute equally to the shared pinball loss; un-scale at inference
# via item_std indexed by query_idx.
...
def _predict_with_unstandardise(fit, batch, thresh):
    ...
    Returns (p_h1_xz, q_hat, preds) matching predict_amortised_p_h1_for_one_xz."""
    ...
    scale = item_std[np.asarray(batch['query_idx']).astype(int)]  # (B,)
...
# Order interims chronologically for x-axis.
_month_order = (
...
#     bar_h^(j*) = sum_j w_j^(j*) q_tok(t_j) in R^E   (cross-attn)
