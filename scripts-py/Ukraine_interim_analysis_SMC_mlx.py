#!/usr/bin/env python3
"""
Ukraine interim analysis: BATCHED SMC on Apple Metal GPU via MLX.

GPU port of ``Ukraine_interim_analysis_with_SMC_resample.py``: all S future
samples share one ``(S, K, D)`` particle tensor on the GPU, each sample has its
own β_s advancing independently (per-sample masking — no shared worst-case
schedule), and the MALA move is one fused Metal kernel. The partial-credit
log-likelihood and prior live in MLX (`python/mlx_pcm.py`); the SMC orchestration
(β bisection, systematic resample, endpoint labels) stays on CPU/JAX.

Outputs mirror the other interim scripts (``_SMCmlx`` suffix).

Usage:
    cd /Users/or105/git/bIRTistic
    pixi run python scripts-py/Ukraine_interim_analysis_SMC_mlx.py
"""
...
from fit_interim_mlx import fit_interim_SMC_batched_GPU_PPS_of_posterior_xz_from_x
...
dir_out = "/Users/or105/sandbox/bIRTistic/py-ukraine-interim-with-SMC-mlx-260526"
...
# Batched-MLX SMC fitting-method arguments. Per-sample beta masking inside ->
# no need to over-temper across samples.
...
for interim_id in di['interim_id']:
    ...
    if interim_m <= 0:
        continue
    print(f"\n{'='*70}\nMLX-SMC-PPS for interim {interim_id} ({interim_date.date()})\n{'='*70}")
    ...
    p, schedule = fit_interim_SMC_batched_GPU_PPS_of_posterior_xz_from_x(
        xi=xi, zi=zi, dit=dit,
    ...
    print(f"  interim {interim_id} MLX-SMC-PPS done in {mins_interim_id:.2f} min ({len(schedule)} temps)")

if not p_h1_xz_rows:
    raise RuntimeError("[MLX-SMC-PPS] no interims with missing participants to evaluate.")
...
print(f"\nAll interims MLX-SMC-PPS done in {mins_total:.2f} min")

sched_all.to_csv(os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_schedule.csv"), index=False)
pkl_path = os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_p_h1_xz.pkl")
p_h1_xz.to_pickle(pkl_path)
print(f"Saved MLX-SMC P(H_1 | x, z) samples to: {pkl_path}")
pps_timing.to_csv(os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_timing.csv"), index=False)
...
csv_path = os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx.csv")
pps_df.to_csv(csv_path, index=False)
print(f"Saved MLX-SMC PPS table to: {csv_path}")
...
print("\nPlotting MLX-SMC p(H_1 | x, z) distribution...")
tmp = p_h1_xz.merge(
...
pkl_path = os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_p_h1_xz_boxplot.pkl")
box_stats.to_pickle(pkl_path)
print(f"Saved MLX-SMC p(H_1 | x, z) box-stats to: {pkl_path}")
...
p = (
    ...
    + labs(x='Interim', y='p(H_1 | x, z)  [SMC MLX]')
)
box_pdf = os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_p_h1_xz_boxplot.pdf")
p.save(box_pdf, verbose=False, limitsize=False)
print(f"Saved MLX-SMC p(H_1 | x, z) boxplot to: {box_pdf}")
...
print("\nBuilding MLX-SMC performance long-form table...")
...
perf_long['method'] = 'SMC (MLX GPU)'
pkl_path = os.path.join(dir_out, f"{file_prefix}_pps_SMCmlx_perf_long.pkl")
perf_long.to_pickle(pkl_path)
print(f"Saved MLX-SMC performance long-form to: {pkl_path}")

print("\n✓ MLX-SMC interim analysis complete")
