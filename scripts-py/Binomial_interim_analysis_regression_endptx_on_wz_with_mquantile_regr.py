...
2014), continuous-target variant with **multi-quantile regression** (Rao-Blackwellised)
...
with linear ``statsmodels.QuantReg`` at 11 quantile levels (0.05..0.95).
Predicted quantiles are stacked into a monotone CDF and interpolated
at eta_0 to produce a **continuous** P(H_1|x,z) in [0,1].

Outputs (``_RGEM_`` suffix so compare-methods can load them next to the
...
Usage:
    ...
    pixi run python scripts-py/Binomial_interim_analysis_regression_endptx_on_wz_with_mquantile_regr.py
"""
...
dir_out = "/Users/or105/sandbox/bIRTistic/py-binomial-interim-with-regression-on-endptx-wz-mquantile-regr-260702"
...
#   4. fit_interim_regress_endptx_on_wz_with_mquantile_regr(wa)
# =============================================================================
...
box_stats.to_pickle(os.path.join(dir_out, f"{file_prefix}_pps_RGEM_p_h1_xz_boxplot.pkl"))
...
tmp = os.path.join(dir_out, f"{file_prefix}_pps_RGEM_p_h1_xz_boxplot.pdf")
...
bs['method'] = 'Regression endptx mquantile'
...
tmp = pd.concat(
    [
        ...
        tmp[['interim_id', 'interim_date', 'pps']].assign(method='Regression endptx mquantile'),
    ],
    ignore_index=True,
)
tmp['interim_date'] = pd.to_datetime(tmp['interim_date'])
tmp['method'] = pd.Categorical(
    tmp['method'], categories=['analytic', 'Regression endptx mquantile'], ordered=True,
)
tmp = tmp.merge(
    bs[['interim_id', 'method', 'q025', 'q975']],
    on=['method', 'interim_id'], how='left',
)
tmp.to_pickle(os.path.join(dir_out, f"{file_prefix}_pps_RGEM_bootstrap.pkl"))

_bar_colours = {'analytic': '#000000', 'Regression endptx mquantile': '#8c564b'}
p = (
    ggplot(
        tmp,
...
tmp = os.path.join(dir_out, f"{file_prefix}_pps_RGEM_bars.pdf")
p.save(tmp, verbose=False, limitsize=False)
...
print("\n✓ Binomial regression-endptx-mquantile PPS + diagnostic plot complete.")
