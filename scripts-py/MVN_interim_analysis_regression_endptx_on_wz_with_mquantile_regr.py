...
2014), continuous-endpoint variant with **multi-quantile regression** (Rao-Blackwellised)
...
with linear ``statsmodels.QuantReg`` at 11 quantile levels
(0.05..0.95). Predicted quantiles are stacked into a monotone CDF and
interpolated at eta_0 to produce a **continuous** P(H_1|x,z) in [0,1].

Outputs per J (``_RGEM_`` suffix so compare-methods can load them next to
...
Usage:
    ...
    pixi run python scripts-py/MVN_interim_analysis_regression_endptx_on_wz_with_mquantile_regr.py
"""
...
DIR_OUT = os.path.join(
    ...
    "py-mvn-interim-with-regression-on-endptx-wz-mquantile-regr-260702",
)
...
for J in J_GRID:
    ...
    perf_all.to_csv(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_perf.csv'),
                    index=False)
    timing_all.to_csv(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_timing.csv'),
                      index=False)
    pps_df.to_csv(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM.csv'),
                  index=False)
    wa_all.to_pickle(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_wa.pkl'))
    an_all.to_pickle(os.path.join(DIR_OUT,
                                  f'mvn_J{J}_pps_RGEM_p_h1_xz_analytic.pkl'))
    rge_artifacts[J] = {
...
bs_B = 2000
METHOD_LABEL = 'Regression of endpt-x on w(z) - mquantile'
...
for J in J_GRID:
    ...
    box.to_pickle(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_p_h1_xz_box.pkl'))
    ...
    pdf_path = os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_p_h1_xz.pdf')
    ...
    bar.to_pickle(os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_bars.pkl'))
    ...
    pdf_path = os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_bars.pdf')
    p.save(pdf_path, verbose=False, limitsize=False)
    ...
    pdf_path = os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_bars_all.pdf')
    p.save(pdf_path, verbose=False, limitsize=False)
    ...
    pdf_path = os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_p_h1_xz_all.pdf')
    p.save(pdf_path, verbose=False, limitsize=False)
    ...
    scatter.to_pickle(
        os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_scatter.pkl'),
    )
    ...
    pdf_path = os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_scatter.pdf')
    p.save(pdf_path, verbose=False, limitsize=False)
    ...
    rge_stats.to_pickle(
        os.path.join(DIR_OUT, f'mvn_J{J}_pps_RGEM_stats.pkl'),
    )
    ...
    p.save(pdf_path, verbose=False, limitsize=False)
    print(f"J={J}: saved RGEM diagnostic scatter to {pdf_path}")
