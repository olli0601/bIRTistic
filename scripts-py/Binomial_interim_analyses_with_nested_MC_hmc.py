...
dir_out = "/Users/or105/sandbox/bIRTistic/py-binomial-interim-with-nested-MC-hmc-260605"
...
#   3. For each s, refit the posterior on (xi, zi_s) via NumPyro NUTS
#      (fit_pyro_hmc) and record p(H_1 | x, z_s) = fraction of posterior
...
for i in range(len(di)):
    ...
    # Outer interim fit (HMC) -> zarr with posterior['ypred'] feeding zi.
...
p = (
    ...
    + geom_boxplot(stat='identity', fill='#d62728', alpha=0.4, colour='#808080')
    ...
    + labs(
        x='Interim', y='p(H_1 | x, z) for predicted z samples',        
    )
)
...
# #d62728 bar = HMC nested-MC PPS. Dodged side-by-side. Vertical bars on
# the HMC bar = 95% bootstrap interval (B resamples with replacement of the
# S MC samples per interim, q025/q975 over the resulting bootstrap PPS dist).
# =============================================================================
...
tmp = pd.concat(
    [
        ...
        tmp[['interim_id', 'interim_date', 'pps']].assign(method='HMC'),
    ],
    ignore_index=True,
)
tmp['interim_date'] = pd.to_datetime(tmp['interim_date'])
tmp['method'] = pd.Categorical(
    tmp['method'], categories=['analytic', 'HMC'], ordered=True,
)
tmp = tmp.merge(
...
_bar_colours = {'analytic': '#000000', 'HMC': '#d62728'}
p = (
    ggplot(
        tmp,
