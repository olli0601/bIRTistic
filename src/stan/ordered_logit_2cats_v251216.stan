...
model {
  ...
  for (n in 1 : Ncat1) {
    target += ordered_logistic_lupmf(cat1_y[n] | cat1_eta[n], cat1_cutpoints[cat1_question_of_obs[n], :]');
  }
  for (n in 1 : Ncat2) {
    target += ordered_logistic_lupmf(cat2_y[n] | cat2_eta[n], cat2_cutpoints[cat2_question_of_obs[n], :]');
  }
...
generated quantities {
  ...
  cat2_ordered_prob_by_obs = ol_get_ordered_prob_by_obs(cat2_eta,
                                                        ...
                                                        cat2_question_of_obs);
  for (n in 1 : Ncat1) {
    cat1_ypred[n] = ordered_logistic_rng( cat1_eta[n], cat1_cutpoints[cat1_question_of_obs[n], :]');    
  }
  for (n in 1 : Ncat2) {
    cat2_ypred[n] = ordered_logistic_rng( cat2_eta[n], cat2_cutpoints[cat2_question_of_obs[n], :]');
  }
  for (n in 1 : Ncat1) {
    log_lik[n] = ordered_logistic_lpmf(cat1_y[n] | cat1_eta[n], cat1_cutpoints[cat1_question_of_obs[n], :]');
  }
  for (n in 1 : Ncat2) {
    log_lik[Ncat1 + n] = ordered_logistic_lpmf(cat2_y[n] | cat2_eta[n], cat2_cutpoints[cat2_question_of_obs[n], :]');
  }
}
