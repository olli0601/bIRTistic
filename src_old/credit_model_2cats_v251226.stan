...
transformed data {
  ...
  s2z_sd_kcat1 = inv(sqrt(1. - inv(Kcat1)));
  s2z_sd_kcat2 = inv(sqrt(1. - inv(Kcat2)));
}
parameters {
  sum_to_zero_vector[U] latent_factor_unit;
  vector[P] latent_factor_beta;
  array[Qcat1] sum_to_zero_vector[Kcat1] cat1_skill_thresholds;
  vector<lower=0>[Qcat1 - 1] cat1_loadings_questions_m1;
  array[Qcat2] sum_to_zero_vector[Kcat2] cat2_skill_thresholds;
...
transformed parameters {
  ...
  cat1_eta = cm_get_etas(append_row(1.0, cat1_loadings_questions_m1),
                         ...
                         cat1_unit_of_obs);
  cat2_eta = cm_get_etas(append_row(1.0, cat2_loadings_questions_m1),
...
model {
  ...
  for (q in 1 : Qcat1) {
    target += normal_lupdf(cat1_skill_thresholds[q] | 0, s2z_sd_kcat1);
  }
  for (q in 1 : Qcat2) {
    target += normal_lupdf(cat2_skill_thresholds[q] | 0, s2z_sd_kcat2);
  }
