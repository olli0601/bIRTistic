...
def _build_test_case():
    # Three category types with heterogeneous K and Q.
    C = 3
    P = 3
    U = 3
    ...
    # Force P_pr < P to exercise ordered_prob_by_cat_qu_pr recomputation.
    ...
    # Flat parameterization exactly matching Stan ncats parameters block.
    skill_thresholds = np.array([
        # c1: Q=2, K-1=2 -> 4 entries
        -0.3, 0.4,
        0.2, 0.9,
        # c2: Q=2, K-1=3 -> 6 entries
        -0.7, -0.1, 0.3,
        0.0, 0.5, 1.1,
        # c3: Q=3, K-1=2 -> 6 entries
        -0.5, 0.2,
        -0.1, 0.6,
        0.3, 1.0,
    ], dtype=float)
    ...
    # NumPyro samples latent_factor_unit_raw then applies sum-to-zero deterministic transform.
    numpyro_substitute = {
        'latent_factor_unit_raw': latent_factor_unit,
        'latent_factor_beta': latent_factor_beta,
        'skill_thresholds': skill_thresholds,
        'loadings_questions_m1_raw': loadings_questions_m1,
